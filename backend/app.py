...
class Handler(BaseHTTPRequestHandler):
    ...
    def check_rate_limit(self, key, limit, window_seconds):
        now = time.time()
        history = RATE_LIMIT_BUCKETS.get(key, [])
        history = [ts for ts in history if now - ts < window_seconds]
        if len(history) >= limit:
            RATE_LIMIT_BUCKETS[key] = history
            return False
        history.append(now)
        RATE_LIMIT_BUCKETS[key] = history
        return True

    def log_audit(self, conn, actor_type, actor_id, action, details=''):
        conn.execute(
            'INSERT INTO audit_logs (actor_type, actor_id, action_name, details) VALUES (?,?,?,?)',
            (actor_type, actor_id, action, details),
        )
    ...
    def do_GET(self):
        ...
        with db_conn() as conn:
            ...
            if path == '/admin/audit-logs':
                ...
                if not admin:
                    return self._json(401, {'error': 'Admin auth required'})
                rows = conn.execute('SELECT id, actor_type, actor_id, action_name, details, created_at FROM audit_logs ORDER BY id DESC LIMIT 100').fetchall()
                return self._json(200, {'logs': [dict(r) for r in rows]})
    ...
    def do_POST(self):
        ...
        with db_conn() as conn:
            ...
            if path == '/qr/scan':
                ...
                if not user:
                    return self._json(401, {'error': 'Unauthorized'})
                if not self.check_rate_limit(f"scan:{user['id']}", 20, 60):
                    return self._json(429, {'error': 'Too many scan requests'})
            ...
            if path == '/submissions':
                ...
                if not user:
                    return self._json(401, {'error': 'Unauthorized'})
                if not self.check_rate_limit(f"submit:{user['id']}", 10, 60):
                    return self._json(429, {'error': 'Too many submission requests'})
            ...
            if path == '/admin/broadcast':
                ...
                self.log_audit(conn, 'admin', admin['id'], 'broadcast_message', msg)
                conn.commit()
    ...
    def do_PATCH(self):
        ...
        with db_conn() as conn:
            if path == '/admin/game/state':
                ...
                self.log_audit(conn, 'admin', admin['id'], 'update_game_state', f"status={next_status};close={bool(next_close)}")
                conn.commit()
            ...
            if path == '/admin/game/final-qr':
                ...
                self.log_audit(conn, 'admin', admin['id'], 'set_final_qr', qr_num)
                conn.commit()
            ...
            if path.startswith('/admin/teams/') and path.endswith('/disqualify'):
                ...
                conn.execute("UPDATE groups SET status='disqualified', disqualified_reason=?, disqualified_at=CURRENT_TIMESTAMP WHERE id=?", (reason, gid))
                self.log_audit(conn, 'admin', admin['id'], 'disqualify_team', f"group={gid};reason={reason}")
                conn.commit()
            ...
            if path.startswith('/admin/qr/'):
                ...
                self.log_audit(conn, 'admin', admin['id'], 'edit_qr', f"qr={qr_number};active={bool(next_active)}")
                conn.commit()
